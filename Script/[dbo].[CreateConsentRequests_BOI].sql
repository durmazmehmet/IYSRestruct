USE [SfdcMasterData]
GO
/****** Object:  StoredProcedure [dbo].[CreateConsentRequests_BOI]    Script Date: 8/20/2020 1:17:43 PM ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[CreateConsentRequests_BOI] 
AS BEGIN 
...
BEGIN 
	...
	-- BOI Permission
	IF CHARINDEX('Email',@BOIPermission) > 0 AND ISNULL(@PersonEmail, '') <> '' 
	BEGIN
		...
		VALUES('BOI',@Id, @BOIIysCode, @BOIBrandCode, CONVERT(VARCHAR, @BOIETKDate__c, 20),@BOIETKSource__c, @PersonEmail, 'BIREYSEL','ONAY', 'EPOSTA', GETDATE());
		IF @IYSType__c <> 'Birey'	
			INSERT INTO SfdcMasterData.dbo.IysConsentRequestTemp
				(CompanyCode, SalesforceId, IysCode, BrandCode, ConsentDate, [Source], Recipient, RecipientType, Status, [Type], CreateDate)
			VALUES('BOI',@Id, @BOIIysCode, @BOIBrandCode, CONVERT(VARCHAR, @BOIETKDate__c, 20),@BOIETKSource__c, @PersonEmail, 'TACIR','ONAY', 'EPOSTA', GETDATE());
	END
	...
	BEGIN
		...
		IF CHARINDEX('Telefon',@BOIPermission) > 0 
		BEGIN
			INSERT INTO SfdcMasterData.dbo.IysConsentRequestTemp
				(CompanyCode, SalesforceId, IysCode, BrandCode, ConsentDate, [Source], Recipient, RecipientType, Status, [Type], CreateDate)
			VALUES('BOI',@Id, @BOIIysCode, @BOIBrandCode, CONVERT(VARCHAR, @BOIETKDate__c, 20),@BOIETKSource__c, @PersonMobilePhone, 'BIREYSEL','ONAY', 'ARAMA', GETDATE());
			IF @IYSType__c <> 'Birey'	
				INSERT INTO SfdcMasterData.dbo.IysConsentRequestTemp
					(CompanyCode, SalesforceId, IysCode, BrandCode, ConsentDate, [Source], Recipient, RecipientType, Status, [Type], CreateDate)
				VALUES('BOI',@Id, @BOIIysCode, @BOIBrandCode, CONVERT(VARCHAR, @BOIETKDate__c, 20),@BOIETKSource__c, @PersonMobilePhone, 'TACIR','ONAY', 'ARAMA', GETDATE());
		END
		
		IF CHARINDEX('SMS',@BOIPermission) > 0 
		BEGIN
			INSERT INTO SfdcMasterData.dbo.IysConsentRequestTemp
				(CompanyCode, SalesforceId, IysCode, BrandCode, ConsentDate, [Source], Recipient, RecipientType, Status, [Type], CreateDate)
			VALUES('BOI',@Id, @BOIIysCode, @BOIBrandCode, CONVERT(VARCHAR, @BOIETKDate__c, 20),@BOIETKSource__c, @PersonMobilePhone, 'BIREYSEL','ONAY', 'MESAJ', GETDATE());
			IF @IYSType__c <> 'Birey'	
				INSERT INTO SfdcMasterData.dbo.IysConsentRequestTemp
					(CompanyCode, SalesforceId, IysCode, BrandCode, ConsentDate, [Source], Recipient, RecipientType, Status, [Type], CreateDate)
				VALUES('BOI',@Id, @BOIIysCode, @BOIBrandCode, CONVERT(VARCHAR, @BOIETKDate__c, 20),@BOIETKSource__c, @PersonMobilePhone, 'TACIR','ONAY', 'MESAJ', GETDATE());
		END
	END
